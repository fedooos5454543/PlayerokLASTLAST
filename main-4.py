...
from playerokapi.enums import EventTypes, ItemStatuses, ChatTypes
...
def save_message_map(tg_message_id: int, chat_id: str, label: str):
    """Запоминает, какому чату Playerok соответствует отправленное в Telegram сообщение —
    чтобы потом можно было ответить покупателю через Reply на это сообщение."""
    now_iso = datetime.now(timezone.utc).isoformat()
    with db() as conn:
        conn.execute(
            "INSERT OR REPLACE INTO message_map (tg_message_id, playerok_chat_id, label, created_at) VALUES (?, ?, ?, ?)",
            (tg_message_id, chat_id, label, now_iso),
        )
        # не даём таблице расти бесконечно — оставляем последние 1000 записей
        conn.execute(
            "DELETE FROM message_map WHERE tg_message_id NOT IN "
            "(SELECT tg_message_id FROM message_map ORDER BY tg_message_id DESC LIMIT 1000)"
        )
        conn.commit()


def get_chat_id_by_tg_message(tg_message_id: int):
    with db() as conn:
        row = conn.execute(
            "SELECT playerok_chat_id, label FROM message_map WHERE tg_message_id = ?", (tg_message_id,)
        ).fetchone()
...
class TelegramNotifier:
    ...
    def send(self, text: str, disable_preview: bool = True):
        """Отправляет сообщение. Возвращает message_id отправленного сообщения (int) или None при ошибке."""
        try:
            resp = requests.post(
    ...
    def get_updates(self, offset=None, timeout=20):
        try:
            ...
            resp = requests.get(f"{self.api_url}/getUpdates", params=params, timeout=timeout + 20)
            data = resp.json()
            if not data.get("ok"):
...
def handle_sale(acc, tg: TelegramNotifier, deal, seen_deal_ids: set):
    ...
    text = (
        ...
        f"💡 Ответьте (Reply) на это сообщение, чтобы написать покупателю."
    )
    tg_message_id = tg.send(text)
    chat = getattr(deal, "chat", None)
    chat_id = getattr(chat, "id", None) if chat else None
    if tg_message_id and chat_id:
        save_message_map(tg_message_id, chat_id, buyer_name)
...
def handle_new_message(acc, tg: TelegramNotifier, event):
    ...
    tg_message_id = tg.send(f"💬 <b>{sender}</b>:\n{text}")

    chat = getattr(event, "chat", None)
    chat_id = getattr(chat, "id", None) if chat else None
    if tg_message_id and chat_id:
        save_message_map(tg_message_id, chat_id, sender)
...
HELP_TEXT = (
    ...
    "💡 Чтобы ответить покупателю — просто сделайте Reply (ответить) прямо на его "
    "пересланное сообщение или на уведомление о продаже. Бот сам поймёт, куда отправить.\n\n"
    "/items — список ваших активных товаров и цен\n"
    "/chats — список активных чатов с покупателями\n"
    "/reply &lt;N&gt; &lt;текст&gt; — альтернативный способ ответить в чат №N (номер из /chats)\n"
    "/report day|week|month — отчёт по продажам за период\n"
    "/peak — в какие часы чаще всего покупают\n"
...
def send_to_playerok_chat(acc, chat_id: str, text: str):
    """
    Отправляет сообщение в чат Playerok.

    Судя по описанию статусов чатов в библиотеке, чат со статусом NEW ("нет ни
    одного прочитанного сообщения") недоступен для отправки сообщений — доступным
    он становится только в статусе FINISHED. Поэтому сначала отдельным запросом
    помечаем чат прочитанным, и только потом отправляем текст (одним запросом
    send_message(..., mark_chat_as_read=True) сервер отдавал "нет доступа").
    """
    try:
        acc.mark_chat_as_read(chat_id)
    except Exception:
        logger.warning("Не удалось пометить чат %s прочитанным (продолжаю пробовать отправить)", chat_id)
...
def _find_interlocutor_username(obj, exclude_id: str):
    """
    Ищет никнейм собеседника внутри объекта чата, не полагаясь на конкретное
    название поля (т.к. оно не задокументировано). Проверяет все атрибуты
    объекта на предмет "похоже на UserProfile" (есть .id и .username) и не
    совпадающие с ID нашего аккаунта — а также заглядывает в списки (на случай
    поля вроде participants/members).
    """
    try:
        attrs = vars(obj)
    except TypeError:
    ...
    def _looks_like_user(value):
        return hasattr(value, "id") and hasattr(value, "username")

    for value in attrs.values():
        if _looks_like_user(value) and getattr(value, "id", None) != exclude_id:
            return getattr(value, "username", None)

    for value in attrs.values():
        if isinstance(value, (list, tuple)):
            for item in value:
                if _looks_like_user(item) and getattr(item, "id", None) != exclude_id:
                    return getattr(item, "username", None)
    return None


def cmd_chats(acc, tg: TelegramNotifier):
    with api_lock:
        # Фильтруем только личные переписки (PM) — в чаты типа NOTIFICATIONS/SUPPORT
        # писать нельзя, и попытка отправить туда сообщение вызывает ошибку доступа.
        page = acc.get_chats(count=24, type=ChatTypes.PM)
    chats = getattr(page, "chats", None) or getattr(page, "items", [])

    if not chats:
        tg.send("Активных чатов с покупателями не найдено.")
        return

    pairs = []
    lines = ["💬 <b>Ваши чаты с покупателями</b>\n"]
    unresolved = 0
    for i, chat in enumerate(chats, 1):
        label = _find_interlocutor_username(chat, acc.id)

        if not label:
            # В списке чатов могут быть неполные данные — пробуем получить чат целиком.
            try:
                with api_lock:
                    full_chat = acc.get_chat(chat.id)
                label = _find_interlocutor_username(full_chat, acc.id)
            except Exception:
                pass

        if not label:
            label = f"чат {chat.id[:8]}"
            unresolved += 1
    ...
    if unresolved:
        lines.append(f"\n⚠️ Для {unresolved} чат(ов) не удалось определить ник — показан ID.")
...
def cmd_reply(acc, tg: TelegramNotifier, args: str):
    ...
    try:
        with api_lock:
            send_to_playerok_chat(acc, chat_id, text)
...
def handle_telegram_quote_reply(acc, tg: TelegramNotifier, reply_to_message_id: int, text: str):
    """Обрабатывает ответ (Reply) на пересланное сообщение/уведомление о продаже —
    отправляет текст в соответствующий чат на Playerok."""
    chat_id, label = get_chat_id_by_tg_message(reply_to_message_id)
    if not chat_id:
        tg.send(
            "Не получилось понять, кому отправить это сообщение — "
            "похоже, я не связывал это сообщение с чатом (например, оно слишком старое).\n"
            "Попробуйте ответить на более свежее уведомление, либо используйте /reply НОМЕР текст (номер — из /chats)."
        )
        return
    ...
    try:
        with api_lock:
            send_to_playerok_chat(acc, chat_id, text)
        tg.send(f"✅ Отправлено покупателю ({label}).")
    except Exception as e:
        logger.exception("Не удалось отправить сообщение в чат %s", chat_id)
        tg.send(f"⚠️ Не удалось отправить сообщение: {e}")
        ...
        try:
            ...
            for update in updates:
                ...
                if text.startswith("/"):
                    ...
                    handle_telegram_quote_reply(acc, tg, reply_to.get("message_id"), text)
                else:
                    tg.send(
                        "Чтобы ответить покупателю — сделайте Reply (ответить) прямо на его "
                        "сообщение или на уведомление о продаже. Либо: /reply НОМЕР текст."
                    )
